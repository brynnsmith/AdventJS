...
function distributeGifts(packOfGifts, reindeers) {
    const weightOfPack = packOfGifts.map(gift => gift.length).reduce((a, b) => a + b, 0)
    const weightLimit = reindeers.map(deer => deer.length * 2).reduce((a, b) => a + b, 0)
    return Math.floor(weightLimit / weightOfPack)
}