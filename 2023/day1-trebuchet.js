...
const input = `7jlncfksix7rjgrpglmn9
...
function sumValues(input) {
  ...
  for (let i = 0; i < inputArr.length; i++) {
    let numbers = Array.from(inputArr[i].matchAll(regex), (match) => match[1]);
    console.log(numbers);
    let digits = numbers.map(el => {
        if (vals[el]) {
            return el = vals[el];
        } else {
            return el;
    ...
    console.log(Number(digits[0] + digits[digits.length-1]));
